...
const LEARNING_PATHS = [
  {
    ...
    bg: "bg-blue-50",
    border: "border-blue-200",
    steps: ["Check electoral roll", "Register if needed (Form 6)", "Find your polling booth", "Know your voter ID alternatives"],
  },
  {
    icon: "record_voice_over",
    ...
    bg: "bg-orange-50",
    border: "border-orange-200",
    steps: ["Eligibility check (age, citizenship)", "File Form 2B nomination", "Pay ₹25,000 security deposit", "Campaign within MCC guidelines"],
  },
  {
    icon: "accessible",
    title: "PwD / Senior Voter",
    color: "text-green-700",
    bg: "bg-green-50",
    border: "border-green-200",
    steps: ["Apply for postal ballot (40%+ disability)", "Request wheelchair / helper at booth", "Home voting if 85+ years old", "Use Braille-enabled EVM"],
  },
];
            ...
            <section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {LEARNING_PATHS.map((path) => (
                  <div
                    key={path.title}
                    className={`bg-white rounded-2xl border ${path.border} p-5`}
                  >
                    <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${path.bg} mb-3`}>
                      <span className={`material-symbols-outlined text-xl ${path.color}`} style={{ fontVariationSettings: "'FILL' 1" }}>{path.icon}</span>
                    </div>
                    <h3 className={`text-sm font-bold mb-3 ${path.color}`}>{path.title}</h3>
                    <ol className="space-y-2">
                      {path.steps.map((step, i) => (
                        <li key={i} className="flex items-start gap-2 text-xs text-on-surface-variant">
                          <span className={`flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${path.bg} ${path.color}`}>
                            {i + 1}
                          </span>
                          {step}
                        </li>
